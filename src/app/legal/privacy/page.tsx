...
export default function PrivacyPage() {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-16 prose prose-slate">
      ...
      <h2>제2조의2 (자체 방문 분석)</h2>
      <p>
        회사는 외부 분석 도구(Google Analytics 등)를 사용하지 않고, 서비스
        개선을 위해 방문 통계를 자체적으로 집계합니다. 이 과정에서 다음을
        준수합니다.
      </p>
      <ul>
        <li>분석 목적의 쿠키를 사용하지 않습니다.</li>
        <li>
          IP 주소·원본 브라우저 정보 등 개인을 식별할 수 있는 정보는 저장하지
          않습니다. (기기 구분은 &ldquo;모바일/데스크톱&rdquo; 수준만 기록)
        </li>
        <li>
          수집 항목: 방문 페이지 경로, 유입 출처(검색·소셜·추천 링크 등),
          클릭한 버튼·링크, 임시 세션 식별자.
        </li>
        <li>
          임시 세션 식별자는 브라우저 임시 저장소(sessionStorage)에 보관되는
          익명 난수로, 브라우저를 닫으면 소멸하며 개인을 식별하지 않습니다.
        </li>
      </ul>
