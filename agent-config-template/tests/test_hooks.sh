...
ISO="AGENT_SECRETS=/nonexistent AGENT_LOG_DIR=$WORK/logs AGENT_STATE_DIR=$WORK/state"
...
OUT="$(env $ISO SLACK_WEBHOOK_URL= sh -c ". '$BASE/hooks/lib/notify.sh'; notify 'test'" 2>&1)"
RC=$?
...
env $ISO SLACK_WEBHOOK_URL="http://127.0.0.1:$PORT_OK" sh -c \
...
env $ISO SLACK_WEBHOOK_URL="http://127.0.0.1:$PORT_BAD" sh -c \
...
env $ISO SLACK_WEBHOOK_URL= sh "$BASE/hooks/guard.sh" test-label sh -c 'exit 7' 2>/dev/null
[ $? -eq 7 ] && pass "종료코드 보존 (7)" || fail "종료코드 왜곡"
env $ISO SLACK_WEBHOOK_URL= sh "$BASE/hooks/guard.sh" only-label 2>/dev/null
[ $? -eq 2 ] && pass "인자 부족 rc=2" || fail "인자 검증 실패"
env $ISO SLACK_WEBHOOK_URL= sh "$BASE/hooks/guard.sh" "bad/label" true 2>/dev/null
[ $? -eq 2 ] && pass "경로문자 라벨 거부 rc=2" || fail "위험 라벨 통과"

echo "[7/8] guard — 한글·탭 섞인 실패 출력이 유효 JSON으로 도착 + 전체 로그 보존"
env $ISO SLACK_WEBHOOK_URL="http://127.0.0.1:$PORT_OK" sh "$BASE/hooks/guard.sh" krtest \
