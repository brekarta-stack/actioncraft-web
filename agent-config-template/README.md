...
│   └── biz-c/             # 사업 C 전용
├── docs/
│   ├── runbook.md         # 매일 10초 / 주 45분 내외 / 월 1회 루틴
│   ├── recovery.md        # 재해 복구 절차 (월 1회 리허설)
│   └── observability.md   # 골든 시그널·대시보드 다이어트·에이전트/커넥터 인벤토리
└── tests/
    ├── test_hooks.sh      # 훅 스크립트 검증 (커밋 전 실행)
    ├── skill-redflags.sh  # 스킬 위험 패턴 기계 검사 (심사 보조)
...
```bash
# 0. 사전 준비 (한 번만)
#    ① GitHub에서 "비어 있는" 비공개 레포 agent-config 생성 (README 생성 체크 해제)
#    ② GitHub 인증 준비: gh auth login  (또는 PAT 발급 후 HTTPS 로그인)
#    ③ git 신원 설정 — 없으면 커밋이 fatal로 실패한다:
...
cd ~/agent-config
git init -b main && git add -A && git commit -m "chore: bootstrap agent-config"
...
#  ⚠️ 같은 값을 패스워드 매니저에도 저장한다 — 항목명은 recovery.md와 일치시킬 것:
#     agent-webhook / agent-anthropic (헤르메스 설치 시 agent-hermes-bot 추가)
...
#  ✅ 이 시점의 정상 결과는 :rotating_light: (hermes=DOWN) 이다 —
#     com.agent.hermes 를 아직 설정하지 않았기 때문. 메시지가 "온다"는 것이 검증 포인트.
#     헤르메스 시작 명령을 채우고 load 한 뒤에야 :green_heart: 가 된다.
...
> 레포의 plist를 고쳐 커밋했다면 재적용이 필요하다 — `launchctl load`는 이미 로드된 서비스를
> 다시 읽지 않으므로 **반드시 unload 후 load** 한다:
> `cp` + `sed` (4단계) → `launchctl unload ~/Library/LaunchAgents/com.agent.<이름>.plist && launchctl load ~/Library/LaunchAgents/com.agent.<이름>.plist`
> (git-sync가 plist 변경을 감지하면 이 명령을 담은 알림을 보낸다.)
