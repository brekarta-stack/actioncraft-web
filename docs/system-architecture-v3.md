...
- 중앙 토큰 관리자(토큰 24h·발급 1분 1회 — 프로세스별 발급 금지) + 전역 쓰로틀러(실전 ~20건/초 계정 전역, `EGW00201` 대응). **토큰 갱신은 "자정 크론" 금지** — 자정(KST)은 미국 정규장 한복판이고 재발급 시 기존 토큰이 무효화됨 → 세션 경계 앵커(KR 개장 전 08:30 / 미국 개장 전 22:00 KST)에서 만료여유 기반 갱신 + 갱신 직후 WS 재구독을 한 트랜잭션으로.
- 매매 엔진의 LLM 분석 호출은 LiteLLM 장애 시에도 살도록 **직접 API 폴백 경로 1개** 보유(엔진 자체는 LLM 없이 동작이 원칙).
...
### 발행·승인·시크릿·백업
- LinkedIn `w_member_social` 심사 2~8주 → **D1 신청**, 승인 전 브리지: **Postiz(NAS Docker) 또는 기승인 SaaS(Buffer)**로 발행 노드만 교체.
- Slack 승인·명령: **Socket Mode 1순위**(아웃바운드 웹소켓만 — 인바운드 포트 0으로 진짜 버튼 승인 가능. v3의 "이모지 폴링" 대체) → 그래도 안 되는 외부 웹훅만 Tailscale Funnel/Cloudflare Tunnel.
- **블로그 플랫폼 확인(D1)**: 네이버 블로그·티스토리는 공식 글쓰기 API 없음(폐지) — 완전 자동발행은 **WordPress(REST API)만** 가능, 네이버는 "초안 자동생성→붙여넣기" 반자동. 셀레니움 자동발행은 계정 저품질/정지 리스크로 금지.
- 시크릿: 패스워드 매니저 = 단일 진실원, 컨테이너 .env는 git 밖. KIS AppKey 1년 만료 갱신을 런북 연례 항목으로.
- 백업 3-2-1 복원: 설정=git / 데이터: **매일 `pg_dump`(실행 중 파일 스냅샷은 정합성 미보장) + n8n 워크플로 export + `N8N_ENCRYPTION_KEY` 별도 보관(이 키 없으면 복원해도 크리덴셜 전멸)** → restic→NAS→**오프사이트(B2)**. 2주차 말 복원 리허설 1회.
- 관제(v3.1 재배치): **Uptime Kuma는 NAS가 아니라 미니에** — 감시자와 최중요 감시 대상(NAS)을 분리해야 NAS 다운을 감지할 수 있다. + **외부 무료 하트비트(healthchecks.io) 1개**로 "미니 자체 사망"까지 커버. 맥 heartbeat와 상호감시 유지. TZ=Asia/Seoul 전 컨테이너 명시.
- 로컬 모델 라이선스: 채택 모델 Qwen 계열은 Apache 2.0으로 사업 산출물 무방. **EXAONE 등 비상업 라이선스 모델은 사업 직결 산출물(리드 스코어링·견적 문안)에 사용 금지.**
...
## 9. v3.1 병합 기록 — 독립 설계(외부 Claude 세션 산출물)와의 교차 비교 (2026-07-22)

동일 요구사항으로 독립 작성된 설계와 비교. **핵심 아키텍처(NAS n8n+LiteLLM 백본 / 구독토큰-운영 분리 / 매매 분석·집행 분리 / KIS 채택 / LinkedIn·YouTube 심사 선신청)는 완전 수렴** — 코어의 교차 검증으로 간주.

**상대 설계에서 채택(v3.1 반영)**: ① Kuma를 미니로(감시자·감시대상 분리) + 외부 하트비트 ② Slack Socket Mode(이모지 폴링 대체) ③ KIS 토큰 세션경계 갱신 ④ pg_dump + N8N_ENCRYPTION_KEY 백업 ⑤ 네이버/티스토리 API 부재 → WordPress 전제 확인 ⑥ Postiz 브리지 ⑦ EXAONE 라이선스 주의 ⑧ 매매엔진 LiteLLM 우회 폴백.

**채택 보류(근거)**: 미니에 상시 소형모델(9B)+임베딩 — 미니는 기존 40잡+음성으로 이관 완료 전까지 여유 없음(이관 후 재검토). Qdrant — pgvector로 시작(컨테이너 최소화), 필요 시 승격. 보이스 스택 전면 재구축(pipecat) — 이미 동작하는 voicebridge가 있어 확장이 우선(교체는 P3). 매매엔진 미니 배치 — 미니 자원 경합·레거시 공존 문제로 NAS 컨테이너 유지.

**상대 설계의 맹점(브라운필드 무지)**: 기존 미니의 40개 launchd 잡·voicebridge·papercraft 서비스의 존재를 모름 → 이관·이중발송 방지·"대체 검증 전 삭제 금지" 전략 부재. 그대로 실행하면 기존 함대와 충돌(아침 브리핑 2개, 슬랙 브리지 2개 등). 하드웨어 칩셋(M3 Ultra/M4)은 근거 없는 추정. 토스증권 API 상세·Sora 중단 일정·pipecat 지연 수치는 본 레포 기준 미검증 — 의존 결정 전 재확인 필요.
