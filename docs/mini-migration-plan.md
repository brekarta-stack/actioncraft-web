# 미니 40잡 카탈로그 (D1 산출물, 2026-07-22 인벤토리 기준)

> ⚠️ **성격 변경(2026-07-22)**: 사용자의 업무 도메인 재편이 예정됨 → 이 문서는 "전량 이관표"가 아니라
> **선별용 카탈로그**다. 기존 잡을 1:1로 n8n에 이식하지 않는다. 도메인 정의 확정 후,
> 살아남는 목적에 해당하는 잡만 골라 **파트 정의 테이블 기반 공용 플로우로 재구축**한다.
> 도메인과 무관한 관제류(§1의 Kuma 흡수 4종)와 백업만 도메인 확정 전에 처리한다.
